...
def admin():
    if request.method == 'POST':
        ...
        if Data is not None:
            ...
            return {"message": "successfully_created"}
        else:
            return "invalid credits"
        ...
        if request.method == 'POST':
            ...
            if user:
                ...
                cur.execute(insert_query,
                            (session['Login_user_mail'], login_time, logout_time, session['Login_user_id']), )
                mysql.connection.commit()
                cur.close()
                token = jwt.encode({'opportunity_id': user['opportunity_id'], 'user_id': user['user_id']}, "secret",
                                   algorithm="HS256")
...
def naga():
    if request.method == 'POST':
        user_id = request.json['user_id']
        cur = mysql.connection.cursor()
        resultVal = cur.execute("select count(OPPORTUNITY_NAME) as OPPORTUNITY_NAME from INVESTED where user_id=%s",
                                (user_id,))
...
def som():
    if request.method == 'POST':
        user_id = request.json['user_id']
        cur = mysql.connection.cursor()
        resultVal = cur.execute("select sum(INVESTMENT_AMOUNT) as INVESTMENT_AMOUNT  from INVESTED where user_id=%s",
                                (user_id,))
...
def user_signup():
    try:
        if request.method == 'POST' and (request.json or request.form):
            ...
            if ob1.userpass_req():
                return 'Password should contain atleast one small letter, one big letter, ' \
                       'one numeric and one special character with 8 minimum letters'
            else:
                print("PASSWORD")
                ...
                cursor.execute(
                    'INSERT INTO signup(FULL_NAME,EMAIL,PHONE_NUMBER,PASSWORD,USER_IP,USER_DATE_CREATED,USER_DEVICE,'
                    'TERMS,PRODUCT_NAME,LOCATION) VALUES (%s,%s, %s, %s,%s, %s,%s,%s,%s,%s)',
                    (ob1.FULL_NAME,
                     ob1.EMAIL,
                     ob1.PHONE_NUMBER,
                     h.hexdigest(),
...
def product():
    if request.method == 'POST' and (request.json or request.form):
        ...
        if request.json:
            ...
            cursor = mysql.connection.cursor()
            cursor.execute(
                "INSERT INTO opportunity(OPPORTUNITY_NAME,OPPORTUNITY_IMAGE,INVESTMENT_AMOUNT,ROI,OPPORTUNITY_TYPE,"
...
def pre():
    ...
    resultVal = cur.execute(
        "SELECT ID,OPPORTUNITY_NAME,OPPORTUNITY_IMAGE ,STARTING_DATE,AREA_NAME,OPPORTUNITY_TYPE,STATUS FROM opportunity")
    if resultVal > 0:
        userDetails = cur.fetchall()
        return json.dumps(userDetails, default=str)
...
def up():
    if request.method == 'PUT':
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            "update opportunity set OPPORTUNITY_NAME=%s,INVESTMENT_AMOUNT=%s,ROI=%s,OPPORTUNITY_TYPE=%s,"
            "OPPORTUNITY_DESC=%s, AREA_NAME=%s,AREA_STANDARD=%s,REVENUE=%s,EXPENSES=%s,TAX=%s,TENANT_NAME=%s,"
...
def disp():
    cur = mysql.connection.cursor()
    resultVal = cur.execute(
        "SELECT ID,ROI,OPPORTUNITY_NAME,INVESTMENT_AMOUNT,AREA_NAME,OPPORTUNITY_TYPE,STATUS FROM opportunity")
    if resultVal > 0:
        userDetails = cur.fetchall()
        return json.dumps(userDetails, default=str)
...
def produce():
    if request.method == 'POST' and (request.json or request.form):
        ...
        if request.json:
            ...
            cursor = mysql.connection.cursor()
            cursor.execute(
                "INSERT INTO portfolio(TOTAL_REVENUE,TOTAL_INVESTED_AMOUNT,PROFIT,TOTAL_NO_OF_INVESTMENTS,OUTLET_NAME)"
                " VALUES""(%s,%s,%s,%s,%s)",
                (ob1.TOTAL_REVENUE,
...
def producee():
    if request.method == 'POST' and (request.json or request.form):
        ...
        if request.json:
            ...
            cursor = mysql.connection.cursor()
            cursor.execute(
                "INSERT INTO pharmacy(CL_PRODUCT_ID,INVESTMENT_PRODUCT_NAME,LOCATION,INVESTMENT_AMOUNT,AMOUNT,GROWTH,"
                "PROFIT_LOSS,REVENUE,EXPENSES) VALUES""(%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                (ob1.CL_PRODUCT_ID,
...
def firstchart():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        resultVal = cur.execute(
            " select first_chart.opportunity_id,first_chart.revenues,first_chart.months,first_chart.expenses from invested inner join first_chart on invested.opportunity_id=first_chart.opportunity_id where invested.user_id=%s",
            (user_id,))
...
def userdetailone():
    if request.method == 'POST':
        ...
        # data = cur.fetchall()
        cur.execute(
            "select first_chart.revenues, first_chart.months from invested inner join first_chart on invested.opportunity_id=first_chart.opportunity_id where invested.opportunity_id=%s",
            (opportunity_id,))
        data1 = cur.fetchall()
...
def secondchart():
    if request.method == 'GET':
        cur = mysql.connection.cursor()
        resultVal = cur.execute(
            "select second_chart.revenues,second_chart.months,second_chart.expenses from second_chart cross join invested where second_chart.opportunity_id=invested.opportunity_id")
...
def thirdchart():
    if request.method == 'GET':
        cur = mysql.connection.cursor()
        resultVal = cur.execute(
            "select third_chart.revenues,third_chart.months,third_chart.expenses from third_chart cross join invested where third_chart.opportunity_id=invested.opportunity_id")
...
def test():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        resultVal = cur.execute(
            " select *,first_chart.months,first_chart.revenue from invested join on first_chart where invested.opportunity_id=first_chart.opportunity_id=%s",
            (opportunity_id,))
